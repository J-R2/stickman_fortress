...
const START_SPAWN_COUNT := 10
const START_TIME_MIN := 0.5
const START_TIME_MAX := 1.5
var min_time :float = START_TIME_MIN
var max_time :float = START_TIME_MAX
var cur_wave_max_spawns :int = START_SPAWN_COUNT
...
func next_wave() -> void :
	...
	next_wave_count_flag += cur_wave_max_spawns
	if current_wave > 0 and current_wave % 2 == 0:
		min_time = max(0.3, min_time - 0.1)
		max_time = max(1.0, max_time - 0.15)
...
func show_wave_banner() -> void :
	...
	label.text = txt
	banner.position = Vector2(534, -95)
	var tween = create_tween()
	tween.tween_property(banner, "position:y", screen_size.y + 150, 2.0)
